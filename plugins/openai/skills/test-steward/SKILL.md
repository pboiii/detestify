...
PostToolUse, SubagentStop, Stop, and SessionEnd. Codex has no native
`TaskCompleted` event, so `task_complete` is advertised unsupported and never
synthesized from Stop. The Stop hook may request at most one remediation
continuation; repeated stops always allow. Hooks are a guardrail, not a
sandbox, and ordinary Chat or Work conversations do not execute them.

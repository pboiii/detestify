...
| OPEN-002 | Exact package versions and transitive licenses | M0 | The exact lockfile, license inventory, and a production CycloneDX 1.5 SBOM with 10 components and 11 dependency entries were verified. The tag workflow regenerates the SBOM; the separate npm workflow requests provenance when publishing the same release tarball. | implementation agent | CLOSED |
| OPEN-003 | Final public name and namespace reservation | Broader promotion and npm publication | The owned GitHub namespace is public at `pboiii/detestify`. Exact npm, PyPI, crates.io, Homebrew, `.com`, and `.dev` checks found no registered collision on 2026-08-31. npm reservation and human legal review remain. | human | OPEN |
